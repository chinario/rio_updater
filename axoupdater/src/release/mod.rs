...
impl AxoUpdater {
    ...
    /// Configures AxoUpdater to use a specific CNB.cool token when performing requests.
...
pub(crate) async fn get_specific_version(
    ...
    let release = match release_type {
        ...
        ReleaseSourceType::CNB => {
            cnb::get_specific_cnb_version(name, owner, app_name, version, &tokens.cnb).await?
        }
        #[cfg(not(feature = "cnb_releases"))]
        ReleaseSourceType::CNB => {
            return Err(AxoupdateError::BackendDisabled {
                backend: "cnb".to_owned(),
            })
        }
    };
...
pub(crate) async fn get_specific_tag(
    ...
    let release = match release_type {
        ...
        #[cfg(feature = "cnb_releases")]
        ReleaseSourceType::CNB => {
            cnb::get_specific_cnb_tag(name, owner, app_name, tag, &tokens.cnb).await?
        }
        #[cfg(not(feature = "cnb_releases"))]
        ReleaseSourceType::CNB => {
            return Err(AxoupdateError::BackendDisabled {
...
pub(crate) async fn get_release_list(
    ...
    let releases = match release_type {
        ...
        #[cfg(feature = "cnb_releases")]
        ReleaseSourceType::CNB => cnb::get_cnb_releases(name, owner, app_name, &tokens.cnb).await?,
        #[cfg(not(feature = "cnb_releases"))]
        ReleaseSourceType::CNB => {
            return Err(AxoupdateError::BackendDisabled {
                backend: "cnb".to_owned(),
